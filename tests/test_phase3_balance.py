...
def make_master():
    return app.MasterData(names={}, places={}, abilities=[
...
def test_catcher_only_normal_special_is_blocked_for_non_catcher():
    specials = app.generate_specials(random.Random(1), make_master(), "野手", "守備職人", position="外野手", age=28,
...
def test_catcher_rank_is_not_generated_for_non_catcher():
    ranked = app.generate_ranked_specials(random.Random(3), make_master(), "野手", "一塁手", "守備職人", {"守備力":{"value":80}}, 28,
                                         category="架空球団用", player_class="一軍主力級", archetype="守備", position_style="守備型一塁手")
    assert "キャッチャー" not in ranked
